...
|**external_id**       |string      |         |*__Agreement__ identifier on the merchant's side. This will be included in the request body of the success / cancel callback. It is not possible to update the external_id. The external_id should be unique to the agreement. Two different agreements should not have the same external_id ||
