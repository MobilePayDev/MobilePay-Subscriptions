...
The client is the application that is attempting to get access to the user's account. The Client is important because it solves the important need that IT-integrators have, Delegated access.  The client needs to get consent from the user before it can do so.This consent is granted through mechanism in the [OpenID Connect](http://openid.net/connect/) protocol suite. <br />
