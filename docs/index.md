...
Send the generated *.cer (or *.crt, if you use OpenSSL) files to [recurringpayments@mobilepay.dk](mailto:recurringpayments@mobilepay.dk) and store the *.pfx file in a secure private key storage on your end.
...
|**currency**          |string(3)   |required |*The __Agreement__ currency code, that will be displayed for the use in the MobilePay app.*|DKK|
|**country_code**      |string(2)   |required |*Country code, which will be used to differentiate between MobilePay DK, NO and FI apps.*|DK|
...
|**frequency**         |int         |required |*Frequency of __Payment Requests__. This value will be used to divide the amount of days in a year to get a frequency in days (e.g. 365 / 12 = 30.4 - approx. every month.)*|12 - every month|
...
#### Agreement creation - single device flow
